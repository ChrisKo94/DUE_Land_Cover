...
def main(hparams):
    ...
    def log_results(trainer):
        ...
        '''
        if trainer.state.epoch > 10 and trainer.state.epoch % 3 == 0:
            _, auroc, aupr, auroc_dsm, aupr_dsm, auroc_msp, aupr_msp = get_ood_metrics(
                hparams.dataset, "LCZ42_Build", model, likelihood, hparams.data_root
            )
            print(f"OoD Metrics - AUROC (PredEnt): {auroc}, AUPR (PredEnt): {aupr}, AUROC (DSM): {auroc_dsm}, AUPR (DSM): {aupr_dsm}, AUROC (MSP): {auroc_msp}, AUPR (MSP): {aupr_msp} ")
        ...
        result += f"Acc: {acc:.4f} "
        print(result)
        #.add_scalar("Loss/test", test_loss, trainer.state.epoch)
    ...
    results["auroc_ood"] = auroc
    results["aupr_ood"] = aupr
    results["auroc_ood_dsm"] = auroc_dsm
    results["aupr_ood_dsm"] = aupr_dsm
    results["auroc_ooc_msp"] = auroc_msp
    results["aupr_ood_msp"] = aupr_msp
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset",
        default="LCZ42_Veg",
        choices=["CIFAR10", "CIFAR100", "LCZ42_Veg", "LCZ42_Build", "EUROSAT_ID"],
        help="Pick a dataset",
    )
    ...
    parser.add_argument(
        "--n_inducing_points", type=int, default=50, help="Number of inducing points"
    )

    parser.add_argument("--seed", default=42, type=int, help="Seed to use for training")

    parser.add_argument(
        "--coeff", type=float, default=6, help="Spectral normalization coefficient"
    )
    ...
    parser.add_argument(
        "--data_root", default="D:/Dateien", type=str, help="Specify data directory"
    )
